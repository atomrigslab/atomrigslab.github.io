// JSON 파일을 불러올 URL
const team_blockchain_rnd_info = './js/blockchain_sector/team_blockchain_rnd_info.json';

// 데이터를 가져와서 HTML에 삽입하는 함수
function displayData_blockchain_rnd(team_blockchain_rnd_info) {
  const container = document.getElementById('team_blockchain_rnd');
  let html = '';
  let card = document.createElement("li");
  for (let item of team_blockchain_rnd_info) {
...
fetch(team_blockchain_rnd_info)
  .then(response => response.json())
  .then(team_blockchain_rnd_info => displayData_blockchain_rnd(team_blockchain_rnd_info))
