// JSON 파일을 불러올 URL
const team_wallet_info = './js/team_wallet_info.json';

// 데이터를 가져와서 HTML에 삽입하는 함수
function displayData_wallet(team_wallet_info) {
  const container = document.getElementById('team_wallet');
  let html = '';
  let card = document.createElement("li");
  for (let item of team_wallet_info) {
...
fetch(team_wallet_info)
  .then(response => response.json())
  .then(team_wallet_info => displayData_wallet(team_wallet_info))
