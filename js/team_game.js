// JSON 파일을 불러올 URL
const team_game_info = './js/team_game_info.json';

// 데이터를 가져와서 HTML에 삽입하는 함수
function displayData_game(team_game_info) {
  const container = document.getElementById('team_game');
  let html = '';
  let card = document.createElement("li");
  for (let item of team_game_info) {
...
fetch(team_game_info)
  .then(response => response.json())
  .then(team_game_info => displayData_game(team_game_info))
