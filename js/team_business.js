// JSON 파일을 불러올 URL
const team_business_info = './js/team_business_info.json';

// 데이터를 가져와서 HTML에 삽입하는 함수
function displayData_business(team_business_info) {
  const container = document.getElementById('team_business');
  let html = '';
  let card = document.createElement("li");
  for (let item of team_business_info) {
...
fetch(team_business_info)
  .then(response => response.json())
  .then(team_business_info => displayData_business(team_business_info))
